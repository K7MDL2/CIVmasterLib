/* 
ICradio_IC905_Simpletest -  W.Dilling/DK8RW

This device is based on a Teensy4 board and shall connect to a standalone IC905 via USB
...
ICradio IC905(TypeIC905,CIV_ADDR_905);
...
byte          G_IC905_state_O   = RADIO_NDEF;
...
void set_KW_Bands() {
  ...
  // get frequency from radio (IC905)
  CAT_freq = IC905.getFrequency()/1000; // frequ in kHz
...
void setup() {
  ...
  civ.setupp(true);                   // initialize the civ object/module (true means "use BT")
	...
	IC905.setupp(millis());            // registering of the CIV-address in civ is done in ICradio
...
void loop() {
  ...
  if ((time_current_baseloop - time_last_baseloop) > BASELOOP_TICK) {
    ...
    if (time_current_baseloop>t_RadioCheck) IC905.loopp(time_current_baseloop);

    // is radio on or off? -> update database
    L_IC905_state = IC905.getAvailability();
    if (G_IC905_state_O!=L_IC905_state) {
      G_IC905_state_O=L_IC905_state;

      if (G_IC905_state_O==RADIO_ON) {
        digitalWrite(P_CONN_RADIO_LED,LOW);   // LED on
      }
      else {
        digitalWrite(P_CONN_RADIO_LED,HIGH);  // LED off
      }
      Serial.print("Radio is: "); Serial.println(DISPLAY_ON_OFF[G_IC905_state_O]);      
    }

    // set the chosen SW-band, if the radio is ON
    if (G_IC905_state_O==RADIO_ON) set_KW_Bands();   //---- RADIO_ON
                                                     // -> setze das KW-Band
